...
class SiswaDoaExport implements FromView, WithStyles
{
    ...
    */
    ...
    private $row_lenght, $doa_column_length;
    ...
    public function view(): View
    {
        ...
        $doa_column_length = count($data_doa);
        $this->doa_column_length = $doa_column_length;
        ...
        return view('siswaDoa.export_excel', [
            ...
            'column_length' => $this->doa_column_length,
        ]);
    }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('D11:' . $this->getColumnIndex($this->doa_column_length + 3) .'10')->getAlignment()->setWrapText(true);
        $sheet->getStyle('D11:' . $this->getColumnIndex($this->doa_column_length + 3) .'10')->getAlignment()->setHorizontal('center');
        $sheet->getStyle('D11:' . $this->getColumnIndex($this->doa_column_length + 3) .'10')->getAlignment()->setVertical('center');
        $sheet->getStyle('D11:' . $this->getColumnIndex($this->doa_column_length + 3) .'10')->getAlignment()->setShrinkToFit(true);

        //Unprotect nilai cell
        $sheet->getStyle('D11:' . $this->getColumnIndex($this->doa_column_length + 3) . $this->row_lenght + 10)->getProtection()->setLocked(Protection::PROTECTION_UNPROTECTED);

        // Add border to range
        $sheet->getStyle('A9:' . $this->getColumnIndex($this->doa_column_length + 3) . $this->row_lenght + 10)->getBorders()->getAllBorders()->setBorderStyle('thin');


        //validation rule for nilai cell as integer between 0-100 and not empty only
        $startCell = 'D11'; // Starting cell for validation
        $endCell = $this->getColumnIndex($this->doa_column_length + 3) . ($this->row_lenght + 10); // Ending cell for validation
        ...
        // Set the range that we want to apply the validation rule to
