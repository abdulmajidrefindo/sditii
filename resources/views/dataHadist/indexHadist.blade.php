...
@section('content_header')
    ...
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0">Hadist</h1>
        </div>
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                {{ Breadcrumbs::render('dataHadist') }}
            </ol>
        </div>
    </div>
@stop
                    ...
                    <div class="card-body">
                        <div class="tab-content" id="hadistTabContent">
                            <div class="tab-pane active show" id="content-tab-hadist-table" role="tabpanel"
                                ...
                                <table id="tabel-hadist" class="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                            <th>Periode</th>
...
<script>
    $(document).ready(function() {
        ...
        }).buttons().container().appendTo('#tabel-hadist_wrapper .col-md-6:eq(0)');
...
<script>
    ...
    $(document).ready(function() {
        $('#kelas_id').on('change', function() {
            let id = $(this).val();
            $('#tabel-hadist').DataTable().destroy();
            $('#tabel-hadist').DataTable({
                "responsive": true,
                "lengthChange": true,
                "autoWidth": false,
...
<script>
    ...
    $(document).on('click', '.delete', function() {
        let hadist_id = $(this).attr('data-id');
        let url = '{{ route('dataHadist.destroy', ':hadist_id') }}';
        url = url.replace(':hadist_id', hadist_id);
        Swal.fire({
            title: 'Apakah anda yakin?',
